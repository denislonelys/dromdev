...
**IIStudio** — мощный AI-оркестратор нового поколения для работы с `arena.ai`.  
...
git clone https://github.com/yourusername/iistudio.git
cd iistudio
...
```bash
npm install -g pm2
pm2 start main.py --interpreter python3 --name iistudio
...
```bash
# Запустить всё (IIStudio + PostgreSQL + Redis + Nginx)
docker-compose up -d
...
# Посмотреть логи
docker-compose logs -f iistudio
...
```bash
# Собрать образ
docker build -t iistudio:latest .

# Запустить только IIStudio (если PostgreSQL и Redis уже есть)
docker run -d \
  --name iistudio \
  -p 8080:8080 \
  -v $(pwd)/.env:/app/.env \
...
# Войти в контейнер
docker exec -it iistudio bash

# Просмотр логов
docker logs -f iistudio
```
...
docker run -d \
  ...
  -e DATABASE_URL=postgresql+asyncpg://user:pass@db:5432/iistudio \
...
IIStudio поддерживает два типа прокси: **MTProto** и **SOCKS5**.
...
```
You > /mode coding
IIStudio > ✅ Режим переключён на: Coding

You > /model deepseek-coder-v2
IIStudio > ✅ Модель изменена: DeepSeek Coder V2

You > Напиши парсер JSON на Rust
IIStudio > 🔄 Отправляю в DeepSeek Coder V2...
           [код через 1.8 сек]

You > /compare Объясни TCP за 2 предложения
IIStudio > 🚀 Сравниваю 3 модели параллельно...
           [таблица результатов]
```
...
# === БАЗА ДАННЫХ ===
DATABASE_URL=postgresql+asyncpg://iistudio:password@localhost:5432/iistudio
...
Доступные метрики:
- `iistudio_requests_total` — общее количество запросов
- `iistudio_response_time_seconds` — время ответа
- `iistudio_cache_hit_rate` — процент попаданий в кэш
- `iistudio_proxy_latency_ms` — задержка прокси
- `iistudio_model_errors_total` — ошибки по моделям
...
**◈ IIStudio — Один инструмент, все нейросети мира ◈**
